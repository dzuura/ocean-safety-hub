...
class Guide {
  constructor(data = {}) {
    ...
    this.conditions = {
      ...
      urgency_levels: data.conditions?.urgency_levels || [] // ['low', 'normal', 'high', 'critical']
    };
  ...
  validate() {
    ...
    if (!['general', 'safety', 'navigation', 'emergency'].includes(this.category)) {
  ...
  matchesConditions(tripConditions) {
    ...
    // Cek setiap kondisi
    const checks = [
      this.conditions.trip_purposes.length === 0 || this.conditions.trip_purposes.includes(trip_purpose),
      this.conditions.boat_types.length === 0 || this.conditions.boat_types.includes(boat_type),
      this.conditions.duration_ranges.length === 0 || this.conditions.duration_ranges.includes(duration_range),
      this.conditions.passenger_ranges.length === 0 || this.conditions.passenger_ranges.includes(passenger_range),
      this.conditions.weather_conditions.length === 0 || this.conditions.weather_conditions.includes(weather_condition),
      this.conditions.urgency_levels.length === 0 || this.conditions.urgency_levels.includes(urgency_level)
    ];
