...
class GuideSession {
  constructor(data = {}) {
    ...
    this.trip_info = {
      ...
      duration_minutes: data.trip_info?.duration_minutes || 0,
      ...
      departure_location: data.trip_info?.departure_location || '',
      destination_location: data.trip_info?.destination_location || '',
      planned_departure_time: data.trip_info?.planned_departure_time || null,
      weather_condition: data.trip_info?.weather_condition || 'calm', // calm, moderate, rough
      urgency_level: data.trip_info?.urgency_level || 'normal' // low, normal, high, critical
    };
    ...
    this.checklist_progress = {
      ...
      completion_percentage: data.checklist_progress?.completion_percentage || 0
    };
  ...
  validateTripInfo() {
    ...
    } else if (!['fishing', 'transport', 'recreation', 'emergency'].includes(trip_info.trip_purpose)) {
      errors.trip_purpose = 'Tujuan perjalanan tidak valid';
    }

    if (!trip_info.duration_minutes || trip_info.duration_minutes < 15) {
      errors.duration_minutes = 'Durasi perjalanan minimal 15 menit';
    } else if (trip_info.duration_minutes > 1440) { // 24 jam
    ...
    } else if (!['perahu_kecil', 'kapal_nelayan', 'kapal_besar'].includes(trip_info.boat_type)) {
      errors.boat_type = 'Jenis perahu tidak valid';
    }

    if (trip_info.planned_departure_time) {
      const departureTime = new Date(trip_info.planned_departure_time);
      const now = new Date();
      if (departureTime < now) {
        errors.planned_departure_time = 'Waktu keberangkatan tidak boleh di masa lalu';
      }
    }
  ...
  updateChecklistProgress(guideId, isCompleted) {
    ...
    if (existingIndex >= 0) {
      ...
      this.checklist_progress.items[existingIndex].completed_at = isCompleted ? new Date().toISOString() : null;
    } else {
