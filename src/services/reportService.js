...
class ReportService {
  constructor() {
    ...
    this.votesCollection = db.collection('report_votes');
  }
  ...
  async searchReports(filters = {}) {
    try {
      ...
      if (filters.verification_status) {
        query = query.where('verification.status', '==', filters.verification_status);
      }
      ...
      if (filters.start_after) {
        const startAfterDoc = await this.collection.doc(filters.start_after).get();
  ...
  async voteOnReport(reportId, userId, voteType, accuracyRating = null) {
    try {
      ...
      console.log(`Vote ${voteType || 'removed'} on report ${reportId} by user ${userId}`);
      return report;
    } catch (error) {
  ...
  async getCommunityReportStats(communityId) {
    try {
      ...
      snapshot.forEach(doc => {
        ...
        if (report.urgency_level === 'high' || report.urgency_level === 'critical') {
      ...
      if (ratedReports > 0) {
        stats.average_accuracy_rating = Math.round((totalRating / ratedReports) * 10) / 10;
      }
